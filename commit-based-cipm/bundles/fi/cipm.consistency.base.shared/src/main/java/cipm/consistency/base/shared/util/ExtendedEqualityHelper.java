...
/**
 * This is an extension/ modification of the {@link EqualityHelper} of the EMF framework. In this
 * implementation we mark objects as visited and do not use a map. In cases of loops in the
 * references of model objects, this implementation provides better results in terms of content
 * equality.
 * 
...
public class ExtendedEqualityHelper {
    ...
    /**
     * Determines whether two EObjects are equal by using the original {@link EqualityHelper} of the
     * EMF. The extension is that already visited objects are not considered. This makes it possible
     * to deal with loops in the models.
     * 
     * @param eObject1
     *            the first model
     * @param eObject2
     ...
     *            set of already visited objects within the models
     * @return true if the objects are equal (including childs, attributes, ...), false otherwise
     */
    public boolean equals(EObject eObject1, EObject eObject2, Set<EObject> visitedObjects) {
        ...
        if (eObject1.eIsProxy()) {
            if (((InternalEObject) eObject1).eProxyURI()
                .equals(((InternalEObject) eObject2).eProxyURI())) {
                visitedObjects.add(eObject1);
                visitedObjects.add(eObject2);
                return true;
            } else {
