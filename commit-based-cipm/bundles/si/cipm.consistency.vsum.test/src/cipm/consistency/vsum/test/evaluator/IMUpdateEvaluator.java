...
public class IMUpdateEvaluator {
    ...
    public void evaluateIMUpdate(Repository repo, InstrumentationModel im, IMEvaluationData evalData, String rootDir) {
        ...
        currentEvalResult
            .setNumberDeactivatedAIP(currentEvalResult.getNumberAIP() - currentEvalResult.getNumberActivatedAIP());
        currentEvalResult.setDeactivatedIPAllIPRatio(
                (double) currentEvalResult.getNumberDeactivatedAIP() / currentEvalResult.getNumberAllIP());
        currentEvalResult.setDeactivatedAIPAllAIPRatio(
                (double) currentEvalResult.getNumberDeactivatedAIP() / currentEvalResult.getNumberAIP());
    ...
    private ServiceInstrumentationPoint findSIP(InstrumentationModel im, ResourceDemandingSEFF seff) {
        ...
        currentEvalResult.getUnmatchedSEFFElements()
            .add(seff.getId());
        return null;
    }
    ...
    private void checkChangedActions(Repository repo1, String rootDir, InstrumentationModel im) {
        ...
        Resource res = set.getResource(URI.createFileURI(new File(rootDir, "Repository.repository").getAbsolutePath()),
                true);
        Repository repo2 = (Repository) res.getContents()
            .get(0);
        ...
        for (var sip : im.getPoints()) {
            for (var aip : sip.getActionInstrumentationPoints()) {
                if (aip.isActive()) {
                    if (newActions.contains(aip.getAction()
                        .getId())) {
                        newActions.remove(aip.getAction()
                            .getId());
                    } else {
                        difference++;
                    }
